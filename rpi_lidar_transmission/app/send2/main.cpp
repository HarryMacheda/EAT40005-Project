...
void send_lidar_data(int sockfd, const std::string &data) {
...
int main(int argc, const char * argv[]) {
    ...
    // start scan...
    drv->startScan(0,1);
    // Setup socket once before scan loop
    ...
    // Create JSON object to store the full 360-degree scan
    Json::Value nodeData(Json::arrayValue);

    // Track previous angle to detect wrap-around
    ...
    while (1) {
		...
		// Add timeframe (UNIX timestamp in milliseconds)
		auto now = std::chrono::system_clock::now();
		auto now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(now);
		node["timeframe"] = static_cast<Json::UInt64>(now_ms.time_since_epoch().count());

		std::string dataString = Json::writeString(writer, node);
		send_lidar_data(sockfd, dataString);
	    }

        }

    if (ctrl_c_pressed) {
    // After exiting the loop, send the remaining scan if any
	if (!nodeData.empty()) {
	    Json::Value fullScan;
	    auto now = std::chrono::system_clock::now();
	    auto now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(now);
	    Json::UInt64 timestamp = static_cast<Json::UInt64>(now_ms.time_since_epoch().count());

	    fullScan["scan"] = nodeData;
	    fullScan["timeframe"] = timestamp;

	    std::string dataString = Json::writeString(writer, fullScan);
	    send_lidar_data(sockfd, dataString);
	}
